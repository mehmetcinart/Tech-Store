    ...
    <div className="container" style={{ padding: "1.5rem 1rem", display: "flex", gap: "1.5rem", alignItems: "flex-start" }}>
      ...
      <aside style={styles.sidebar}>
        <div style={styles.filterBox}>
          ...
          <button
            style={{ ...styles.filterItem, fontWeight: !category ? 700 : 400, color: !category ? "#2C7A5E" : "#2C4F48" }}
            onClick={() => setFilter("category", "")}
          >Tümü</button>
          {CATEGORIES.map((c) => (
            <button key={c}
              style={{ ...styles.filterItem, fontWeight: category === c ? 700 : 400, color: category === c ? "#2C7A5E" : "#2C4F48" }}
        ...
        <div style={styles.filterBox}>
          ...
          <button
            style={{ ...styles.filterItem, fontWeight: !brand ? 700 : 400, color: !brand ? "#2C7A5E" : "#2C4F48" }}
            onClick={() => setFilter("brand", "")}
          >Tümü</button>
          {BRANDS.map((b) => (
            <button key={b}
              style={{ ...styles.filterItem, fontWeight: brand === b ? 700 : 400, color: brand === b ? "#2C7A5E" : "#2C4F48" }}
      ...
      <main style={{ flex: 1 }}>
        <div style={styles.topBar}>
          <div>
            <h1 style={styles.pageTitle}>
              {category || (search ? `"${search}" sonuçları` : "Tüm Ürünler")}
            </h1>
        ...
        {loading ? (
          ...
          <div style={styles.empty}>
            ...
            <p style={{ color: "#8AADA4" }}>Farklı filtreler veya arama terimi deneyin</p>
          </div>
        ) : (
...
const styles = {
  ...
  filterBox:   { background: "#fff", borderRadius: "12px", padding: "1.25rem", boxShadow: "0 1px 3px rgba(44,122,94,.08)" },
  filterTitle: {
    fontSize: ".8rem", fontWeight: 700, textTransform: "uppercase",
    letterSpacing: ".05em", color: "#5E8A80", marginBottom: ".875rem",
  },
